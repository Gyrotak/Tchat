...
<div class="container">
        <div class="row">
       <form method="post">
       <button type="submit" name="deconnexion" class="btn btn-danger" style="margin:auto;margin-bottom:2%;" >Deconnexion</button>
    </form>
<div class="">
...
<?php foreach($allMessages as $message) {
    if ($message["ID_User"] != $_SESSION["id_user"]) { ?>
                            <li class="left clearfix"><span class="chat-img pull-left">
                                <img src="http://placehold.it/50/55C1E7/fff&text=<?= strtoupper($account->getUserByID($message["ID_User"])[0]["Email"][0]) ?>" alt="User Avatar" class="img-circle" />
                            </span>
                                <div class="chat-body clearfix">
                                    <div class="header">
                                                       <strong class="primary-font"><?= $account->getUserByID($message["ID_User"])[0]["Email"]; ?></strong> <small class="pull-right text-muted">
...
<?php foreach($UserConnected as $user) { ?>
